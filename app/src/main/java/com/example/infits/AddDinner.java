...
public class AddDinner extends RecyclerView.Adapter<AddDinner.AddBreakfastViewHolder> {

    List<ModelForFood> breakfastList;
    Context context;
    AddButtonListenerChart addButtonListenerChart;

    public AddDinner(List<ModelForFood> breakfastList, Context context, AddButtonListenerChart addButtonListenerChart) {
...
//        holder.photo.setImageDrawable(context.getResources().getDrawable(R.drawable.add_food));
        ...
        if (position != 0 ){
            if(breakfastList.get(position-1).getPhoto().getConstantState()  != context.getResources().getDrawable(R.drawable.add_food).getConstantState()){
                addButtonListenerChart.addButtonOnClick(String.valueOf(position),"dinner");
            }
            else{
