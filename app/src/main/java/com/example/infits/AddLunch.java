...
public class AddLunch extends RecyclerView.Adapter<AddLunch.AddBreakfastViewHolder> {

    List<ModelForFood> breakfastList;
    Context context;
    AddButtonListenerChart addButtonListenerChart;

    public AddLunch(List<ModelForFood> breakfastList, Context context, AddButtonListenerChart addButtonListenerChart) {
    ...
    public void onBindViewHolder(@NonNull AddBreakfastViewHolder holder, int position) {
        ...
        if (position != 0 ){
            if(breakfastList.get(position-1).getPhoto().getConstantState()  != context.getResources().getDrawable(R.drawable.add_food).getConstantState()){
                addButtonListenerChart.addButtonOnClick(String.valueOf(position),"lunch");
            }
            else{
